...
const DB = {
  ...
  async logConsent(agreed) {
    await this.save('consent', { 
      ...
      buttonOrderHumanFirst: buttonOrderHumanFirst
    });
  },
...
console.info('[Experiment] Block Sequence:', blockSequence, 'Button Order:', buttonOrderHumanFirst ? 'Human First' : 'Robot First');
 
/* =========================================================
   Block configurations (2 experimental blocks)
   Single-lane design: User allocates one package to either human or robot
   ========================================================= */
/* ---------------------------------------------------------
   CONDITION: BALANCED EFFORT CONSTANT
   - Same number of clicks: 12 manual pushes OR 12 error fixes (6×2)
   - Block 1: Robot is FASTER than human
   - Block 2: Human is FASTER than robot
   - Trade-off: Time saved vs Effort type
   --------------------------------------------------------- */
const BLOCKS = {
  1: {
    nLanes: 1,
    stepsToComplete: 12,
    robotMoveCooldownMs: 1000,    // Robot: 24s total (12 steps × 2000ms) - FASTER
    humanMoveCooldownMs: 3000,    // Human: 48s total (12 steps × 4000ms) - SLOWER
    robotErrorsPerPackage: 6,     // 6 errors × 2 clicks = 12 clicks
    humanErrorsPerPackage: 0      // 12 push clicks
  },
  2: {
    nLanes: 1,
    stepsToComplete: 12,
    robotMoveCooldownMs: 3000,    // Robot: 48s total (12 steps × 4000ms) - SLOWER
    humanMoveCooldownMs: 1000,    // Human: 24s total (12 steps × 2000ms) - FASTER
    robotErrorsPerPackage: 6,     // 6 errors × 2 clicks = 12 clicks
    humanErrorsPerPackage: 0      // 12 push clicks
  }
};
...
/* ---------------------------------------------------------
   CONDITION: BALANCED TIME CONSTANT
   - Same total completion time within each block (robot = human = 24s)
   - Effort varies BETWEEN blocks for BOTH agents
   - Block 1: 12 steps (2s each) - Robot LOW effort (4 clicks), Human HIGH effort (12 clicks)
   - Block 2: 6 steps (4s each) - Robot HIGH effort (12 clicks), Human LOW effort (6 clicks)
   - Trade-off: Who has to work harder this round?
   --------------------------------------------------------- */
// const BLOCKS = {
//   1: {
//     nLanes: 1,
//     stepsToComplete: 12,          // 12 steps
//     robotMoveCooldownMs: 2000,    // Robot: 24s total (12 × 2000ms)
//     humanMoveCooldownMs: 2000,    // Human: 24s total (12 × 2000ms) - SAME TIME
//     robotErrorsPerPackage: 2,     // Robot: 2 errors × 2 clicks = 4 clicks - LOW EFFORT
//     humanErrorsPerPackage: 0      // Human: 12 push clicks - HIGH EFFORT
//   },
//   2: {
//     nLanes: 1,
//     stepsToComplete: 6,           // 6 steps (shorter conveyor)
//     robotMoveCooldownMs: 2000,    // Robot: 24s total (6 × 4000ms)
//     humanMoveCooldownMs: 2000,    // Human: 24s total (6 × 4000ms) - SAME TIME
//     robotErrorsPerPackage: 6,     // Robot: 6 errors × 2 clicks = 12 clicks - HIGH EFFORT
//     humanErrorsPerPackage: 0      // Human: 6 push clicks - LOW EFFORT
//   }
// };
...
function assignErrorsForLane(laneIndex) {
  ...
  // Shuffle and pick first N steps for errors
  const errorSteps = [];
  for (let i = 0; i < Math.min(numErrors, availableSteps.length); i++) {
    const randomIndex = Math.floor(Math.random() * availableSteps.length);
    errorSteps.push(availableSteps[randomIndex]);
    availableSteps.splice(randomIndex, 1);
  }
...
function maybeError(i) {
  ...
  // Check if current move count matches any predetermined error step
  if (p.errorSteps.includes(p.moveCount)) {
    p.inError = true;
    
    // Log error occurrence (logic trigger)
    // Note: 'error_occurred' is also logged in setBubble when the UI element appears
    // We keep this one for logic tracking but maybe rename or rely on the UI one
    // To avoid duplicates, let's remove this one and rely on setBubble which represents "Available to user"
    /*
    ...
    steps: [
      {
        ...
        intro: 'You will act as a <strong>warehouse worker</strong>. Your job is to move a package from the storage area into the green delivery zone.<br><br><strong>Your goal: Complete each round as FAST as possible!</strong>'
      },
      {
      ...
      },
      {
        ...
        intro: '<strong>Remember: Work as FAST as possible!<br>Let\'s do a quick practice round!'
      }
    ],
    showProgress: true,
    showBullets: false,
...
function generateAllocationOptions() {
  ...
  // Binary choice: Human or Robot
  let choices = [
    { mode: false, label: humanLabel, icon: 'human_icon.png', theme: 'human' },
    { mode: true, label: robotLabel, icon: 'robot_icon.png', theme: 'robot' }
  ];
  
  // Randomize order based on participant assignment
  if (!buttonOrderHumanFirst) {
    choices.reverse(); // Robot first
  }
...
function init() {
  ...
  if (blockSurveySubmitBtn) {
    // Clear error state when user starts typing (include clicks inputs if present)
    ...
    blockSurveySubmitBtn.addEventListener("click", async () => {
      ...
      // Validate inputs: times + clicks (if clicks inputs are present)
      ...
      // Require time fields
      if (!robotValue || !humanValue) {
        surveyError.textContent = 'Please answer the time questions before continuing.';
      ...
      // If clicks fields exist, require them as well
      if ((surveyRobotClicksInput || surveyHumanClicksInput) && (!robotClicksValue || !humanClicksValue)) {
        surveyError.textContent = 'Please answer the clicks (effort) questions before continuing.';
        surveyError.style.display = 'block';
      ...
      const perceivedRobotClicks = robotClicksValue ? parseInt(robotClicksValue, 10) : null;
      const perceivedHumanClicks = humanClicksValue ? parseInt(humanClicksValue, 10) : null;

      // Validate time ranges
      if (isNaN(perceivedRobotTime) || isNaN(perceivedHumanTime)) {
        surveyError.textContent = 'Please enter valid numbers for the time questions.';
        surveyError.style.display = 'block';
      ...
      // Validate clicks if present
      if (surveyRobotClicksInput || surveyHumanClicksInput) {
        if (isNaN(perceivedRobotClicks) || isNaN(perceivedHumanClicks)) {
          surveyError.textContent = 'Please enter valid numbers for the clicks questions.';
          surveyError.style.display = 'block';
        ...
        if (perceivedRobotClicks < 1 || perceivedHumanClicks < 1 || perceivedRobotClicks > 20 || perceivedHumanClicks > 20) {
          surveyError.textContent = 'Please enter clicks between 1 and 20.';
          surveyError.style.display = 'block';
          if (perceivedRobotClicks < 1 || perceivedRobotClicks > 20) surveyRobotClicksInput?.classList.add('invalid');
          if (perceivedHumanClicks < 1 || perceivedHumanClicks > 20) surveyHumanClicksInput?.classList.add('invalid');
          return;
        }
      }
      ...
      // include clicks/effort answers when available
      if (typeof perceivedRobotClicks !== 'undefined' && perceivedRobotClicks !== null) {
        surveyData.perceivedRobotClicks = perceivedRobotClicks;
      }
      if (typeof perceivedHumanClicks !== 'undefined' && perceivedHumanClicks !== null) {
        surveyData.perceivedHumanClicks = perceivedHumanClicks;
      }
      ...
      if (surveyType === 'block1') {
        ...
        blockTransitionText.innerHTML = `
          ...
          Your goal is to finish delivering packages as quickly as possible.
        `;
